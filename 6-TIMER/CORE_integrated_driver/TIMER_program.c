...
void TIMER0_voidInit(void)
{
...
#elif Operation_Mode == Fast_PWM //continue on compare match value to overflow 
	...
	SET_BIT(TCCR0,5);
#endif
#elif Operation_Mode == Phase_Correct_PWM //continue on compare match value to overflow 
	//choose Phase_Correct_PWM mode
	SET_BIT(TCCR0,TCCR0_WGM00);
	CLR_BIT(TCCR0,TCCR0_WGM01);
	// no interrupt in phase correct
	/*modes in Phase_Correct_PWM are :
	1-set on upcounting , clear on downcounting
	2-clear on upcounting , set on downcounting
	*/

#endif
}

/*to put the value in timer 0 register for preloading if there is a fractional number of overflow time
...
/*this is mode 14 in timer 1 to put my own top value 
to make a signal with a specific period time to use it to generate PWM signal*/
...
u8 WDT_voidSleep(u8 Copy_u8SleepTime) //Watch Dog Timer
{
