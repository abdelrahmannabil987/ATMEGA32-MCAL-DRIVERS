...
//selecting ADC_Trigger is Free_Running
...
{
	u8 ReadingContainer = Data_Result;
	...
	if (ADC_u8Busy_State==IDLE)
	{
...
#if AUTO_Trigger == disabled //single conversion
		/*Start Conversion*/ 
		...
		else
		{
			...
			if(ReadingContainer == 1) //Left Adjust to read 8 bits 
			{
				/*Return the reading*/
				*Copy_Ptru16Reading =  ADCH;
			}
			else if(ReadingContainer == 2) //Right Adjust to read 10 bits 
			{
...
s32 Mapping(s32 Input_MIN,s32 Input_MAX,s32 Output_MIN,s32 Output_MAX,s32 Input_Value)//A function of a 
straight line
{
...
void __vector_16 (void)
{
	...
	{
		/*Read ADC Result*/
		if(ReadingContainer == Left_Adj) //_8_BITS
		{
			/*Return the reading*/
			*ADC_pu16Reading =  ADCH;
		}
		else if(ReadingContainer == Right_Adj)//_10_BITS
		{
